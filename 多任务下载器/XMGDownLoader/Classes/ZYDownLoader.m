...
- (void)downLoader:(NSURL *)url {
    ...
    if ([url isEqual:self.dataTask.originalRequest.URL]) {
        ...
        // 下载失败
        // 判断当前的状态, 如果是暂停状态
        if (self.state == ZYDownLoadStatePause) {
            // 继续
    ...
    //     return
    if ([ZYFileTool fileExists:self.downLoadedPath]) {
        // UNDO: 告诉外界, 已经下载完成;
//        NSLog(@"已经下载完成");
        self.state = ZYDownLoadStatePauseSuccess;
        return;
    }
    ...
    // 2.2 不存在: 从0字节开始请求资源
    //     return
    if (![ZYFileTool fileExists:self.downLoadingPath]) {
        // 从0字节开始请求资源
...
- (void)pauseCurrentTask {
    if (self.state == ZYDownLoadStateDownLoading) {
...
- (void)resumeCurrentTask {
    if (self.dataTask && self.state == ZYDownLoadStatePause) {
...
-(void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    if (_tmpSize == _totalSize) {
        // 1. 移动到下载完成文件夹
//        NSLog(@"移动文件到下载完成");
        [ZYFileTool moveFile:self.downLoadingPath toPath:self.downLoadedPath];
        // 2. 取消本次请求
...
//        NSLog(@"有问题--%zd--%@", error.code, error.localizedDescription);
        ...
        if (-999 == error.code) {
            ...
            self.state = ZYDownLoadStatePauseFailed;
        }
...
#pragma mark - 事件/数据传递
- (void)setState:(ZYDownLoadState)state {
    // 数据过滤
    if(_state == state) {
        return;
    }
    ...
    if (_state == ZYDownLoadStatePauseSuccess && self.successBlock) {
        self.successBlock(self.downLoadedPath);
    }
    
    if (_state == ZYDownLoadStatePauseFailed && self.faildBlock) {
        self.faildBlock();
    }
